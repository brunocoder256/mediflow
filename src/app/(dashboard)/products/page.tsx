                ...
                {pageRows.map(p=>{
                  ...
                  <Card key={p.id} className="overflow-hidden">
                    <CardContent className="p-4 space-y-2">
                      ...
                      <div className="flex gap-1"><Button size="sm" variant="outline" className="flex-1" disabled={pending} title={pending?"Pending sync": undefined} onClick={()=>openDetail(p.id)}><Eye className="h-4 w-4 mr-1"/>View</Button><Button size="sm" variant="outline" className="flex-1" disabled={pending} title={pending?"Pending sync": undefined} onClick={()=>handleEdit(p as any)}><Edit className="h-4 w-4 mr-1"/>Edit</Button><Button size="sm" variant="outline" className="flex-1 text-destructive hover:text-destructive" disabled={pending} title={pending?"Pending sync": undefined} onClick={()=>handleDeactivate(p.id, p.is_active)}>{p.is_active ? <><Trash2 className="h-4 w-4 mr-1"/>Delete</> : <><History className="h-4 w-4 mr-1"/>Reactivate</>}</Button></div>
                    </CardContent>
                  </Card>
                );})}
              </div>
