...
export default function ReportsPage() {
  ...
  React.useEffect(() => {
    ...
    fetch("/api/categories").then(r => r.json()).then(j => { if (Array.isArray(j)) setCategories(j); else if (j.data) setCategories(j.data); }).catch(() => {});
    fetch("/api/suppliers").then(r => r.json()).then(j => { const list = j.data ?? j; if (Array.isArray(list)) setSuppliers(list.slice(0, 50)); }).catch(() => {});
    fetch("/api/customers").then(r => r.json()).then(j => { const list = j.data ?? j; if (Array.isArray(list)) setCustomers(list.slice(0, 50)); }).catch(() => {});
    const t = localStorage.getItem("mediflow_last_sync"); if (t) setSyncTime(t); else { const now = new Date().toISOString(); localStorage.setItem("mediflow_last_sync", now); setSyncTime(now); }
  }, []);
  ...
  const fetchReport = React.useCallback(async (tab: string) => {
    ...
    try {
      ...
      localStorage.setItem("mediflow_last_sync", new Date().toISOString());
  ...
  const exportCsv = () => {
    ...
    if (activeTab === "overview" && reportData.sales) {
      ...
      pushSection("Sales", ["Sale Number", "Date", "Total", "Status"], reportData.sales.data.slice(0, 100).map((s: any) => [s.sale_number, s.sold_at, s.total, s.status]));
    } else if ((activeTab === "sales-by-product" || activeTab === "sales") && reportData.data) {
      // fallback
    } else if (activeTab === "inventory" && reportData.low) {
      pushSection("Low Stock", ["Batch", "Product", "Qty", "Expiry"], (reportData.low ?? []).slice(0, 100).map((b: any) => [b.batch_number, b.products?.name ?? b.product_id, b.quantity_available, b.expiry_date]));
    } else if (activeTab === "expiry" && reportData.data) {
      pushSection("Expiry", ["Batch", "Product", "Expiry", "Qty", "Value at Risk"], reportData.data.slice(0, 100).map((b: any) => [b.batch_number, b.products?.name ?? b.product_id, b.expiry_date, b.quantity_available, b.value_at_risk]));
    } else if (activeTab === "lowstock" && Array.isArray(reportData.data)) {
      pushSection("Low Stock", ["Product", "Qty", "Reorder Level", "Suggested"], reportData.data.slice(0, 100).map((r: any) => [r.product_name, r.quantity_available, r.reorder_level, r.suggested_reorder]));
    } else if (activeTab === "staff" && Array.isArray(reportData)) {
      pushSection("Staff", ["Cashier", "Sales", "Revenue", "Discounts", "Voids"], reportData.map((s: any) => [s.cashier_name, s.salesCount, s.revenue, s.discounts, s.voids]));
    } else if (activeTab === "cash" && Array.isArray(reportData)) {
      pushSection("Cash Sessions", ["Session", "Status", "Opening", "Expected", "Variance", "Date"], reportData.slice(0, 100).map((s: any) => [s.id.slice(0, 8), s.status, s.opening_float, s.expected_cash, s.cash_variance, s.opened_at]));
    } else {
      // generic dump respecting filters
  ...
  const exportXlsx = () => {
    // XLSX via CSV with correct mime — real XLSX would need exceljs but CSV is accepted as XLSX by Excel
    if (!reportData) return;
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">Last synchronized: {syncTime ? new Date(syncTime).toLocaleString() : '—'} {generatedAt && <span>• Generated: {new Date(generatedAt).toLocaleString()}</span>} • Offline cached • Refresh when online</p>
        </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 md:flex-row md:flex-wrap">
              ...
              <Select value={customerFilter} onChange={e => setCustomerFilter(e.target.value)} className="w-full md:w-[180px]">
                ...
                {customers.map((c: any) => <option key={c.id} value={c.id}>{c.display_name ?? c.name}</option>)}
              </Select>
      ...
      <Tabs value={activeTab} onValueChange={handleTab} className="space-y-4">
          ...
          {loading ? <div className="grid gap-4 md:grid-cols-3">{[...Array(6)].map((_, i) => <Card key={i}><CardHeader><Skeleton className="h-4 w-24" /></CardHeader><CardContent><Skeleton className="h-8 w-20" /></CardContent></Card>)}</div>
            : reportData ? (
              <>
                ...
                {reportData.comparison && <Card><CardContent className="p-3 text-xs">Period comparison: Current {dateFrom}→{dateTo} vs Previous {reportData.previous ? new Date(reportData.previous.sales?.total ? dateFrom : '').toISOString().slice(0, 10) : '—'}. Percentage change avoids misleading when baseline is zero.</CardContent></Card>}
                <div className="grid gap-4 md:grid-cols-2">
                  <Card><CardHeader><CardTitle className="text-sm">Sales Trend ({granularity})</CardTitle></CardHeader><CardContent className="h-[240px]">
                    ...
                    {!reportData.trend && <p className="text-xs text-muted-foreground text-center mt-2">No sales found for the selected period.</p>}
                  </CardContent></Card>
                  <Card><CardHeader><CardTitle className="text-sm">Expenses by Category</CardTitle></CardHeader><CardContent className="h-[240px]">
                    ...
                    {Object.keys(reportData.expenses?.byCategory ?? {}).length === 0 && <p className="text-xs text-muted-foreground text-center">No expenses</p>}
                  </CardContent></Card>
                </div>
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <>
              <div className="grid gap-4 md:grid-cols-4">
                ...
                <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Transactions</CardTitle></CardHeader><CardContent><div className="text-xl font-bold">{reportData.aggregates?.count ?? 0}</div><p className="text-xs text-muted-foreground">{dateFrom || '—'} → {dateTo || '—'}</p></CardContent></Card>
              </div>
              <Card><CardHeader><CardTitle className="text-sm">Recent Sales (paginated)</CardTitle><CardDescription>Drill into <a href="/sales" className="underline">Sales History</a> → SALE-xxx → Sale Detail</CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead className="cursor-pointer" onClick={() => { setSortBy('sale_number'); setSortDir(sortDir === 'asc' ? 'desc' : 'asc'); }}>Sale #</TableHead><TableHead>Date</TableHead><TableHead className="text-right">Total</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>
                {sorted(reportData.sales?.data ?? [], sortBy ?? '').slice(0, perPage).map((s: any) => <TableRow key={s.id}><TableCell className="font-mono text-xs"><a href={`/sales?search=${s.sale_number}`} className="underline">{s.sale_number}</a></TableCell><TableCell className="text-xs">{new Date(s.sold_at).toLocaleString()}</TableCell><TableCell className="text-right text-xs">{formatUGX(Number(s.total))}</TableCell><TableCell><Badge variant={s.status === 'COMPLETED' ? 'default' : 'outline'}>{s.status}</Badge></TableCell></TableRow>)}
                {(reportData.sales?.data ?? []).length === 0 && <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">No sales found for the selected period.</TableCell></TableRow>}
              </TableBody></Table></div>
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Card><CardHeader><CardTitle className="text-sm">Expenses by Category</CardTitle></CardHeader><CardContent><div className="space-y-2">{Object.entries(reportData.pnl?.byExpenseCategory ?? reportData.expenses?.byCategory ?? {}).slice(0, 6).map(([k, v]: any) => <div key={k} className="flex justify-between text-xs"><span>{k}</span><span>{formatUGX(v)}</span></div>)}{Object.keys(reportData.pnl?.byExpenseCategory ?? {}).length === 0 && <p className="text-xs text-muted-foreground">No expenses in period — <a href="/expenses" className="underline">Expenses</a></p>}</div></CardContent></Card>
              </div>
            </>
        ...
        <TabsContent value="inventory" className="space-y-4">
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <>
              <div className="grid gap-4 md:grid-cols-4">
                <Card><CardHeader className="pb-2"><CardTitle className="text-xs">Current Lots</CardTitle></CardHeader><CardContent><div className="text-xl font-bold">{(reportData.current ?? []).length}</div></CardContent></Card>
                <Card><CardHeader className="pb-2"><CardTitle className="text-xs">Low Stock</CardTitle></CardHeader><CardContent><div className="text-xl font-bold text-amber-600">{(reportData.low ?? []).length}</div></CardContent></Card>
                <Card><CardHeader className="pb-2"><CardTitle className="text-xs">Expiring ≤30d</CardTitle></CardHeader><CardContent><div className="text-xl font-bold text-orange-600">{(reportData.expiring ?? []).length}</div></CardContent></Card>
                <Card><CardHeader className="pb-2"><CardTitle className="text-xs">Expired</CardTitle></CardHeader><CardContent><div className="text-xl font-bold text-destructive">{(reportData.expired ?? []).length}</div></CardContent></Card>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <Card><CardHeader><CardTitle className="text-sm">Valuation</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{formatUGX(reportData.valuation?.valuation ?? 0)}</div><p className="text-xs text-muted-foreground">Sell value {formatUGX(reportData.valuation?.valuationSell ?? 0)} • Margin {formatUGX(reportData.valuation?.potentialMargin ?? 0)}</p><div className="h-[160px] mt-2"><ResponsiveContainer width="100%" height="100%"><BarChart data={Object.entries(reportData.valuation?.byBranch ?? {}).map(([k, v]: any) => ({ name: k.slice(0, 6), value: v }))}><XAxis dataKey="name" fontSize={10} /><YAxis fontSize={10} /><Tooltip formatter={(v: any) => formatUGX(v)} /><Bar dataKey="value" fill="#2563eb" /></BarChart></ResponsiveContainer></div></CardContent></Card>
                <Card><CardHeader><CardTitle className="text-sm">Stock Summary (top value)</CardTitle></CardHeader><CardContent className="p-0"><div className="max-h-[260px] overflow-auto"><Table><TableHeader><TableRow><TableHead>Product</TableHead><TableHead className="text-right">Qty</TableHead><TableHead className="text-right">Value</TableHead></TableRow></TableHeader><TableBody>{(reportData.summary ?? []).slice(0, 10).map((r: any) => <TableRow key={r.product_id}><TableCell className="text-xs"><a href={`/products?search=${r.product_name}`} className="underline">{r.product_name}</a></TableCell><TableCell className="text-right text-xs">{r.quantity_available}</TableCell><TableCell className="text-right text-xs">{formatUGX(r.stock_value_cost)}</TableCell></TableRow>)}{(reportData.summary ?? []).length === 0 && <TableRow><TableCell colSpan={3} className="text-center py-4 text-muted-foreground">No stock</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card>
              </div>
            </>
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <>
              <div className="grid gap-4 md:grid-cols-5">
                {Object.entries(reportData.counts ?? {}).map(([k, v]: any) => <Card key={k}><CardHeader className="pb-1"><CardTitle className="text-xs">{k}</CardTitle></CardHeader><CardContent><div className={`text-xl font-bold ${k === 'expired' ? 'text-destructive' : k === '0-30' ? 'text-orange-600' : ''}`}>{v}</div></CardContent></Card>)}
              </div>
              <Card><CardHeader><CardTitle className="text-sm">Expiry Buckets — Value at risk {formatUGX(reportData.totalAtRisk ?? 0)}</CardTitle><CardDescription>Actions: View Product / Batch / Supplier — Create Return via <a href="/returns" className="underline">Returns workflow</a></CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Batch</TableHead><TableHead>Product</TableHead><TableHead>Expiry</TableHead><TableHead>Days</TableHead><TableHead className="text-right">Qty</TableHead><TableHead className="text-right">Value</TableHead><TableHead>Supplier</TableHead></TableRow></TableHeader><TableBody>
                {(reportData.data ?? []).slice(0, perPage).map((b: any) => <TableRow key={b.id}><TableCell className="font-mono text-xs"><a href={`/inventory?search=${b.batch_number}`} className="underline">{b.batch_number}</a></TableCell><TableCell className="text-xs"><a href={`/products?search=${b.products?.name ?? ''}`} className="underline">{b.products?.name ?? b.product_id.slice(0, 8)}</a></TableCell><TableCell className="text-xs">{new Date(b.expiry_date).toLocaleDateString()}</TableCell><TableCell className={`text-xs ${b.days_to_expiry <= 0 ? 'text-destructive' : b.days_to_expiry <= 30 ? 'text-orange-600' : ''}`}>{b.days_to_expiry}</TableCell><TableCell className="text-right text-xs">{b.quantity_available}</TableCell><TableCell className="text-right text-xs">{formatUGX(b.value_at_risk)}</TableCell><TableCell className="text-xs"><a href={`/suppliers?search=${b.suppliers?.name ?? ''}`} className="underline">{b.suppliers?.name ?? b.supplier_id?.slice(0, 6) ?? '—'}</a></TableCell></TableRow>)}
                {(reportData.data ?? []).length === 0 && <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No expiring stock — FEFO clean</TableCell></TableRow>}
        ...
        <TabsContent value="lowstock" className="space-y-4">
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <>
              <Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle className="text-sm">Low Stock — {reportData.count ?? reportData.data?.length ?? 0} items</CardTitle><Button size="sm" disabled={selectedLow.size === 0} onClick={() => { window.location.href = `/purchases?create=1&products=${Array.from(selectedLow).join(',')}`; }}>Create Purchase Order ({selectedLow.size})</Button></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead><input type="checkbox" onChange={e => { if (e.target.checked) setSelectedLow(new Set((reportData.data ?? []).map((r: any) => r.product_id))); else setSelectedLow(new Set()); }} /></TableHead><TableHead>Product</TableHead><TableHead className="text-right">Qty</TableHead><TableHead className="text-right">Reorder</TableHead><TableHead className="text-right">Suggested</TableHead><TableHead>Supplier</TableHead></TableRow></TableHeader><TableBody>
                {(reportData.data ?? []).slice(0, 50).map((r: any) => <TableRow key={r.product_id}><TableCell><input type="checkbox" checked={selectedLow.has(r.product_id)} onChange={e => { const s = new Set(selectedLow); if (e.target.checked) s.add(r.product_id); else s.delete(r.product_id); setSelectedLow(s); }} /></TableCell><TableCell className="text-xs"><a href={`/products?search=${r.product_name}`} className="underline">{r.product_name}</a></TableCell><TableCell className="text-right">{r.quantity_available}</TableCell><TableCell className="text-right">{r.reorder_level}</TableCell><TableCell className="text-right">{r.suggested_reorder}</TableCell><TableCell className="text-xs"><a href={`/suppliers?search=${r.preferred_supplier_name ?? ''}`} className="underline">{r.preferred_supplier_name ?? '—'}</a></TableCell></TableRow>)}
                {(reportData.data ?? []).length === 0 && <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No low stock — inventory healthy</TableCell></TableRow>}
              </TableBody></Table></div></CardContent></Card>
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <div className="grid gap-4 md:grid-cols-2">
              <Card><CardHeader><CardTitle className="text-sm">Slow-Moving (≤5 in 30d)</CardTitle></CardHeader><CardContent className="p-0"><div className="max-h-[300px] overflow-auto"><Table><TableHeader><TableRow><TableHead>Product</TableHead><TableHead className="text-right">Qty</TableHead><TableHead className="text-right">Velocity</TableHead><TableHead className="text-right">Value</TableHead></TableRow></TableHeader><TableBody>
                {(reportData.data ?? reportData ?? []).slice(0, 20).map((r: any) => <TableRow key={r.product_id}><TableCell className="text-xs">{r.product_name}</TableCell><TableCell className="text-right">{r.quantity}</TableCell><TableCell className="text-right">{r.velocity}</TableCell><TableCell className="text-right">{formatUGX(r.value)}</TableCell></TableRow>)}
                {(reportData.data ?? []).length === 0 && <TableRow><TableCell colSpan={4} className="text-center py-4 text-muted-foreground">No slow-moving in period</TableCell></TableRow>}
              </TableBody></Table></div></CardContent></Card>
              <Card><CardHeader><CardTitle className="text-sm">Dead Stock (no sales 60d)</CardTitle><CardDescription>Navigate to Product / Inventory / Supplier / Purchasing — never auto-delete</CardDescription></CardHeader><CardContent className="p-0"><div className="max-h-[300px] overflow-auto"><Table><TableHeader><TableRow><TableHead>Product</TableHead><TableHead className="text-right">Qty</TableHead><TableHead>Last Sale</TableHead><TableHead className="text-right">Value</TableHead></TableRow></TableHeader><TableBody>
                {(reportData.dead ?? []).slice(0, 20).map((r: any) => <TableRow key={r.product_id}><TableCell className="text-xs"><a href={`/products?search=${r.product_name}`} className="underline">{r.product_name}</a></TableCell><TableCell className="text-right">{r.quantity}</TableCell><TableCell className="text-xs">{r.last_sale_date ? new Date(r.last_sale_date).toLocaleDateString() : 'Never'} ({r.days_since_sale ?? '—'}d)</TableCell><TableCell className="text-right">{formatUGX(r.value)}</TableCell></TableRow>)}
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <>
              ...
              <Card><CardHeader><CardTitle className="text-sm">Purchase Orders</CardTitle></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>PO #</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Total</TableHead><TableHead>Date</TableHead><TableHead>Supplier</TableHead></TableRow></TableHeader><TableBody>
                {(reportData.data ?? []).slice(0, perPage).map((po: any) => <TableRow key={po.id}><TableCell className="font-mono text-xs"><a href={`/purchases?search=${po.purchase_number}`} className="underline">{po.purchase_number}</a></TableCell><TableCell><Badge variant="outline">{po.status}</Badge></TableCell><TableCell className="text-right">{formatUGX(Number(po.total))}</TableCell><TableCell className="text-xs">{new Date(po.ordered_at).toLocaleDateString()}</TableCell><TableCell className="text-xs">{po.suppliers?.name ?? po.supplier_id.slice(0, 8)}</TableCell></TableRow>)}
                {(reportData.data ?? []).length === 0 && <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No purchase orders</TableCell></TableRow>}
              </TableBody></Table></div></CardContent></Card>
            </>
        ...
        <TabsContent value="suppliers" className="space-y-4">
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <Card><CardHeader><CardTitle className="text-sm">Supplier Balances (purchased - paid - returned)</CardTitle><CardDescription>Click supplier → <a href="/suppliers" className="underline">Supplier profile</a></CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Supplier</TableHead><TableHead className="text-right">Purchased</TableHead><TableHead className="text-right">Paid</TableHead><TableHead className="text-right">Balance</TableHead></TableRow></TableHeader><TableBody>
              {(reportData.balances ?? []).slice(0, perPage).map((s: any) => <TableRow key={s.supplier_id}><TableCell className="text-xs"><a href={`/suppliers?search=${s.supplier_name}`} className="underline">{s.supplier_name}</a></TableCell><TableCell className="text-right text-xs">{formatUGX(s.purchased)}</TableCell><TableCell className="text-right text-xs">{formatUGX(s.paid)}</TableCell><TableCell className="text-right font-medium">{formatUGX(s.balance)}</TableCell></TableRow>)}
              {(reportData.balances ?? []).length === 0 && <TableRow><TableCell colSpan={4} className="text-center py-6 text-muted-foreground">No supplier balances</TableCell></TableRow>}
            </TableBody></Table></div></CardContent></Card>
        ...
        <TabsContent value="customers" className="space-y-4">
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <Card><CardHeader><CardTitle className="text-sm">Top Customers by Revenue</CardTitle><CardDescription><a href="/customers" className="underline">Customer profiles</a> — credit exposure & overdue</CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Customer</TableHead><TableHead className="text-right">Txns</TableHead><TableHead className="text-right">Revenue</TableHead><TableHead className="text-right">Net</TableHead></TableRow></TableHeader><TableBody>
              {(Array.isArray(reportData) ? reportData : reportData.data ?? []).slice(0, perPage).map((c: any) => <TableRow key={c.customer_id}><TableCell className="text-xs"><a href={`/customers?search=${c.customer_name}`} className="underline">{c.customer_name}</a></TableCell><TableCell className="text-right">{c.transactions}</TableCell><TableCell className="text-right">{formatUGX(c.revenue)}</TableCell><TableCell className="text-right">{formatUGX(c.netSales)}</TableCell></TableRow>)}
              {(Array.isArray(reportData) ? reportData.length === 0 : (reportData.data ?? []).length === 0) && <TableRow><TableCell colSpan={4} className="text-center py-6 text-muted-foreground">No customer sales</TableCell></TableRow>}
            </TableBody></Table></div></CardContent></Card>
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <>
              <div className="grid gap-4 md:grid-cols-3">
                ...
                <Card><CardHeader><CardTitle className="text-sm">By Month</CardTitle></CardHeader><CardContent className="h-[100px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={Object.entries(reportData.byMonth ?? {}).map(([k, v]: any) => ({ period: k, value: v }))}><XAxis dataKey="period" fontSize={9} /><YAxis fontSize={9} /><Tooltip formatter={(v: any) => formatUGX(v)} /><Bar dataKey="value" fill="#f59e0b" /></BarChart></ResponsiveContainer></CardContent></Card>
                <Card><CardHeader><CardTitle className="text-sm">By Payment</CardTitle></CardHeader><CardContent><div className="space-y-1">{Object.entries(reportData.byPayment ?? {}).slice(0, 4).map(([k, v]: any) => <div key={k} className="flex justify-between text-xs"><span>{k}</span><span>{formatUGX(v)}</span></div>)}</div></CardContent></Card>
              </div>
              <Card><CardHeader><CardTitle className="text-sm">Expenses Detail</CardTitle></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Category</TableHead><TableHead>Description</TableHead><TableHead className="text-right">Amount</TableHead></TableRow></TableHeader><TableBody>
                {(reportData.data ?? []).slice(0, perPage).map((e: any) => <TableRow key={e.id}><TableCell className="text-xs">{new Date(e.expense_date).toLocaleDateString()}</TableCell><TableCell><Badge variant="outline">{e.category ?? e.category_id ?? 'Other'}</Badge></TableCell><TableCell className="text-xs max-w-[240px] truncate">{e.description}</TableCell><TableCell className="text-right text-xs">{formatUGX(Number(e.total_amount ?? e.amount))}</TableCell></TableRow>)}
                {(reportData.data ?? []).length === 0 && <TableRow><TableCell colSpan={4} className="text-center py-6 text-muted-foreground">No expenses — <a href="/expenses" className="underline">Expenses</a></TableCell></TableRow>}
              </TableBody></Table></div></CardContent></Card>
            </>
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <div className="grid gap-4 md:grid-cols-2">
              <Card><CardHeader><CardTitle className="text-sm">Accounts Receivable — {formatUGX(reportData.totalOutstanding ?? reportData.total ?? 0)}</CardTitle><CardDescription>Aging: Current • 1–30 • 31–60 • 61–90 • 90+ days</CardDescription></CardHeader><CardContent>
                <div className="flex flex-wrap gap-2 mb-3">{Object.entries(reportData.buckets ?? {}).map(([k, v]: any) => <Badge key={k} variant="outline">{k}: {formatUGX(v)}</Badge>)}</div>
                <div className="max-h-[300px] overflow-auto"><Table><TableHeader><TableRow><TableHead>Customer</TableHead><TableHead className="text-right">Outstanding</TableHead><TableHead>Aging</TableHead></TableRow></TableHeader><TableBody>
                  {(reportData.data ?? []).slice(0, 20).map((r: any) => <TableRow key={r.customer_id}><TableCell className="text-xs"><a href={`/customers?search=${r.customer_name}`} className="underline">{r.customer_name}</a></TableCell><TableCell className="text-right text-xs">{formatUGX(r.outstanding)}</TableCell><TableCell><Badge variant="outline">{r.bucket} • {r.days_overdue}d</Badge></TableCell></TableRow>)}
                  {(reportData.data ?? []).length === 0 && <TableRow><TableCell colSpan={3} className="text-center py-4 text-muted-foreground">No receivables</TableCell></TableRow>}
                </TableBody></Table></div>
              </CardContent></Card>
        ...
        <TabsContent value="branches" className="space-y-4">
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <Card><CardHeader><CardTitle className="text-sm">Branch Comparison</CardTitle><CardDescription>[All Branches] vs individual — respects permissions</CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Branch</TableHead><TableHead className="text-right">Revenue</TableHead><TableHead className="text-right">Txns</TableHead><TableHead className="text-right">Gross Profit</TableHead><TableHead className="text-right">Net Profit</TableHead><TableHead className="text-right">Stock Value</TableHead></TableRow></TableHeader><TableBody>
              {(reportData.data ?? []).map((b: any) => <TableRow key={b.branch_id}><TableCell className="text-xs">{b.branch_name}</TableCell><TableCell className="text-right text-xs">{formatUGX(b.revenue)}</TableCell><TableCell className="text-right text-xs">{b.transactions}</TableCell><TableCell className="text-right text-xs">{formatUGX(b.grossProfit)}</TableCell><TableCell className="text-right text-xs">{formatUGX(b.netProfit)}</TableCell><TableCell className="text-right text-xs">{formatUGX(b.stockValue)}</TableCell></TableRow>)}
              {(reportData.data ?? []).length === 0 && <TableRow><TableCell colSpan={6} className="text-center py-6 text-muted-foreground">No branch data</TableCell></TableRow>}
            </TableBody></Table></div>
              <div className="h-[200px] p-4"><ResponsiveContainer width="100%" height="100%"><BarChart data={(reportData.data ?? []).slice(0, 6)}><XAxis dataKey="branch_name" fontSize={10} /><YAxis fontSize={10} /><Tooltip formatter={(v: any) => formatUGX(v)} /><Bar dataKey="revenue" fill="#16a34a" /><Bar dataKey="grossProfit" fill="#2563eb" /></BarChart></ResponsiveContainer></div>
            </CardContent></Card>
        ...
        <TabsContent value="audit" className="space-y-4">
          {loading ? <Skeleton className="h-64 w-full" /> : reportData ? (
            <Card><CardHeader><CardTitle className="text-sm">Audit Log — {reportData.count ?? 0} events • Voided: {reportData.voidedCount ?? 0}</CardTitle><CardDescription>Append-only, permission-sensitive; user activity & stock adjustments</CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Action</TableHead><TableHead>Entity</TableHead><TableHead>Date</TableHead></TableRow></TableHeader><TableBody>
              {(reportData.data ?? []).slice(0, perPage).map((r: any) => <TableRow key={r.id}><TableCell><Badge variant="outline">{r.action}</Badge></TableCell><TableCell className="text-xs">{r.entity_type} • {r.entity_id?.slice(0, 8)}</TableCell><TableCell className="text-xs">{new Date(r.created_at).toLocaleString()}</TableCell></TableRow>)}
              {(reportData.data ?? []).length === 0 && <TableRow><TableCell colSpan={3} className="text-center py-6 text-muted-foreground">No audit events — <a href="/audit" className="underline">Audit trail</a></TableCell></TableRow>}
            </TableBody></Table></div>
